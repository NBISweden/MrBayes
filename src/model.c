...
void AllocateCppEvents (Param *p)
{
    ...
    p->nEvents = (int **) calloc (2*numGlobalChains, sizeof (int *));
	p->nEvents[0] = (int *) calloc (2*numGlobalChains*(2*numLocalTaxa), sizeof (int));
	for (i=1; i<2*numGlobalChains; i++)
		p->nEvents[i] = p->nEvents[i-1] + (2*numLocalTaxa);
	p->position = (MrBFlt ***) calloc (2*numGlobalChains, sizeof (MrBFlt **));
	p->position[0] = (MrBFlt **) calloc (2*numGlobalChains*(2*numLocalTaxa), sizeof (MrBFlt *));
	for (i=1; i<2*numGlobalChains; i++)
		p->position[i] = p->position[i-1] + (2*numLocalTaxa);
	p->rateMult = (MrBFlt ***) calloc (2*numGlobalChains, sizeof (MrBFlt **));
	p->rateMult[0] = (MrBFlt **) calloc (2*numGlobalChains*(2*numLocalTaxa), sizeof (MrBFlt *));
	for (i=1; i<2*numGlobalChains; i++)
...

{
	...
	if ((temp = (MCMCMove *) calloc (1, sizeof (MCMCMove))) == NULL)
		return (NULL);
    ...
    if ((temp->name = (char *) calloc (nameLength, sizeof (char))) == NULL)
		{
		free (temp);
		return NULL;
		}
	...
	if ((temp->tuningParam = (MrBFlt **) calloc (numGlobalChains, sizeof (MrBFlt *))) == NULL)
		{
	...
	if ((temp->tuningParam[0] = (MrBFlt *) calloc (moveType->numTuningParams*numGlobalChains, sizeof (MrBFlt))) == NULL)
		{
...

{
	...
	for (k=0; k<numParams; k++)
        ...
        if (params[k].paramType == P_TOPOLOGY && params[k].paramId == TOPOLOGY_SPECIESTREE)
			...
			numSubParamPtrs += 2;
		else if (params[k].paramType == P_IBRBRANCHRATES)
			numSubParamPtrs += 2;
		}
	...
	for (k=0; k<numParams; k++)
		...
		if (p->paramType == P_CPPEVENTS)
			...
			q->nSubParams++;
			q = modelSettings[p->relParts[0]].psiGamma;
			q->nSubParams++;
...

{
    ...
    cppEventParams = (Param *) calloc (nCppEventParams, sizeof(Param));
                ...
                for (i=i1=0; i<numParams; i++)
                    ...
                    if (p->paramType == P_CPPEVENTS)
                        ...
                        for (j=0; j<2*numLocalTaxa; j++)
                            ...
                            toPosition[j] = (MrBFlt *) realloc ((void *)toPosition[j], toEvents[j]*sizeof(MrBFlt));
                            toRateMult[j] = (MrBFlt *) realloc ((void *)toRateMult[j], toEvents[j]*sizeof(MrBFlt));
...

{
	...
	mcmcTree = (Tree **) realloc ((void *) mcmcTree, numTrees * 2 * numGlobalChains * sizeof (Tree *));
	if (mcmcTree == NULL)
		{
		memAllocs[ALLOC_MCMCTREES] = NO;
    ...
    for (i1=0; i1<numParams; i1++)
        ...
        if (p->paramType == P_CPPEVENTS)
            {
            p->nEvents = (int **) realloc ((void *)p->nEvents, 2*numGlobalChains*sizeof (int *));
	        p->nEvents[0] = (int *) realloc ((void *)p->nEvents[0], 2*numGlobalChains*(2*numLocalTaxa)*sizeof (int));
	        for (i=1; i<2*numGlobalChains; i++)
		        p->nEvents[i] = p->nEvents[i-1] + (2*numLocalTaxa);
            if (from > to)
                {
        	...
        	p->position = (MrBFlt ***) realloc ((void *)p->position, 2*numGlobalChains*sizeof (MrBFlt **));
            p->position[0] = (MrBFlt **) realloc ((void *)p->position[0], 2*numGlobalChains*(2*numLocalTaxa)*sizeof (MrBFlt *));
        	for (i=1; i<2*numGlobalChains; i++)
		        p->position[i] = p->position[i-1] + (2*numLocalTaxa);
        	p->rateMult = (MrBFlt ***) realloc ((void *)p->rateMult, 2*numGlobalChains*sizeof (MrBFlt **));
        	p->rateMult[0] = (MrBFlt **) realloc ((void *)p->rateMult[0], 2*numGlobalChains*(2*numLocalTaxa)*sizeof (MrBFlt *));
        	for (i=1; i<2*numGlobalChains; i++)
		        p->rateMult[i] = p->rateMult[i-1] + (2*numLocalTaxa);
            if (to > from)
                {
...

{
	...
	int	i, j, st, isCalibrated, isClock;
	...
	if (!strcmp(mp->brlensPr,"Clock") && !strcmp(mp->nodeAgePr,"Calibrated"))
		isCalibrated = YES;
	else
		isCalibrated = NO;
...

{
	...
	if (expecting == Expecting(PARAMETER))
		...
		if (!strcmp(parmName, "Tratio"))
		...
		else if (!strcmp(parmName, "Psigammashape"))
			{
			for (i=0; i<numCurrentDivisions; i++)
				tempLinkUnlink[P_PSIGAMMASHAPE][i] = tempLinkUnlinkVec[i];
			}
			...
			{
			for (i=0; i<numCurrentDivisions; i++)
				tempLinkUnlink[P_IBRBRANCHRATES][i] = tempLinkUnlinkVec[i];
			}
		else
			{
			...
			if (expecting == Expecting(EQUALSIGN))
				...
				if (IsArgValid(tkn, tempStr) == NO_ERROR)
					...
					for (i=0; i<numCurrentDivisions; i++)
						{
						if (activeParts[i] == YES || nApplied == 0)
							strcpy(modelParams[i].psiGammaPr, tempStr);
						}
					}
				else
					{
					MrBayesPrint ("%s   Invalid Psigammapr argument\n", spacer);
					return (ERROR);
					}
				expecting  = Expecting(LEFTPAR);
				for (i=0; i<numCurrentDivisions; i++)
				...
				for (i=0; i<numCurrentDivisions; i++)
					...
					if (activeParts[i] == YES || nApplied == 0)
						{
						if (!strcmp(modelParams[i].psiGammaPr,"Uniform"))
							{
							sscanf (tkn, "%lf", &tempD);
							modelParams[i].psiGammaUni[numVars[i]++] = tempD;
							if (numVars[i] == 1)
								expecting  = Expecting(COMMA);
							else
								{
								if (modelParams[i].psiGammaUni[0] >= modelParams[i].psiGammaUni[1])
									{
									MrBayesPrint ("%s   Lower value for uniform should be greater than upper value\n", spacer);
									return (ERROR);
									}
								if (nApplied == 0 && numCurrentDivisions == 1)
									MrBayesPrint ("%s   Setting Psigammapr to Uniform(%1.2lf,%1.2lf)\n", spacer, modelParams[i].psiGammaUni[0], modelParams[i].psiGammaUni[1]);
								else
									MrBayesPrint ("%s   Setting Psigammapr to Uniform(%1.2lf,%1.2lf) for partition %d\n", spacer, modelParams[i].psiGammaUni[0], modelParams[i].psiGammaUni[1], i+1);
								expecting  = Expecting(RIGHTPAR);
								}
							}
						else if (!strcmp(modelParams[i].psiGammaPr,"Exponential"))
							{
							sscanf (tkn, "%lf", &tempD);
							modelParams[i].psiGammaExp = tempD;
							if (nApplied == 0 && numCurrentDivisions == 1)
								MrBayesPrint ("%s   Setting Psigammapr to Exponential(%1.2lf)\n", spacer, modelParams[i].psiGammaExp);
							else
								MrBayesPrint ("%s   Setting Psigammapr to Exponential(%1.2lf) for partition %d\n", spacer, modelParams[i].psiGammaExp, i+1);
							expecting  = Expecting(RIGHTPAR);
							}
						else if (!strcmp(modelParams[i].psiGammaPr,"Fixed"))
							{
							sscanf (tkn, "%lf", &tempD);
							if (tempD < ALPHA_MIN || tempD > ALPHA_MAX)
								{
								MrBayesPrint ("%s   Psigammashape (alpha) must be in the range %f - %f\n", spacer, ALPHA_MIN, ALPHA_MAX);
								return (ERROR);
								}
							modelParams[i].psiGammaFix = tempD;
							if (nApplied == 0 && numCurrentDivisions == 1)
								MrBayesPrint ("%s   Setting Psigammapr to Fixed(%1.2lf)\n", spacer, modelParams[i].psiGammaFix);
							else
								MrBayesPrint ("%s   Setting Psigammapr to Fixed(%1.2lf) for partition %d\n", spacer, modelParams[i].psiGammaFix, i+1);
							expecting  = Expecting(RIGHTPAR);
							}
						}
					}
...

{
	...
	int                 i, j, k, nMatches, tempInt, treeIndex;
	char		        temp[100];
	...
	if (expecting == Expecting(PARAMETER))
			...
			for (i=0; i<chainParams.numRuns; i++)
				...
				for (j=0; j<chainParams.numChains; j++)
					...
					if (chainIndex != -1 && j != chainIndex)
						continue;
					theTree = GetTree (param, i*chainParams.numChains+j, 0);
					usrTree = GetTree (param, i*chainParams.numChains+j, 1); /* use as scratch space */
					...
					if (param->paramType == P_TOPOLOGY)
                        ...
                        FillTopologySubParams (param, j, 0, &globalSeed);
						}
						...
						FillBrlensSubParams (param, j, 0);
						}
					else if (param->paramType == P_CPPEVENTS || param->paramType == P_BMBRANCHRATES || param->paramType == P_IBRBRANCHRATES)
						{
						...
						if (SetRelaxedClockParam (param, j, 0, userTree[treeIndex]) == ERROR)
							{
							MrBayesPrint ("%s   Could not set parameter '%s' from user tree '%s'\n", spacer, param->name, userTree[treeIndex]->name);
							return (ERROR);
							}
		...
		if (param->printParam == NO && !(param->paramType == P_TOPOLOGY && strcmp(modelParams[param->relParts[0]].topologyPr,"Fixed")!=0)
                                    ...
                                    && !param->paramType == P_BMBRANCHRATES)
			{
			MrBayesPrint ("%s   The parameter '%s' is fixed so the starting value cannot be set\n", spacer, param->name);
			return (ERROR);
			}
		if (param->paramType == P_BRLENS || param->paramType == P_TOPOLOGY || param->paramType == P_CPPEVENTS || param->paramType == P_BMBRANCHRATES || param->paramType == P_IBRBRANCHRATES)
			{
...

{
	...
	for (chn=fromChain; chn<toChain; chn++)
		...
		for (k=0; k<numParams; k++)
			...
			else if (p->paramType == P_PSIGAMMASHAPE)
				{
				/* Fill in psigamma shape (for relaxed clock rate multiplier) ***********************************************************/
				if (p->paramId == PSIGAMMASHAPE_UNI)
					value[0] = RandomNumber(seed) * (mp->psiGammaUni[1] - mp->psiGammaUni[0]) + mp->psiGammaUni[0];
				
				else if (p->paramId == PSIGAMMASHAPE_EXP)
					value[0] =   (-(1.0/mp->psiGammaExp) * log(1.0 - RandomNumber(seed)));
...

{
	...
	for (i=0; i<param->nSubParams; i++)
		...
		else if (q->paramType == P_BMBRANCHRATES || q->paramType == P_IBRBRANCHRATES)
			{
...

{
	...
	for (chn=fromChain; chn<toChain; chn++)
		...
		for (k=0; k<numParams; k++)
			{
			p = &params[k];
			if (p->paramType == P_TOPOLOGY && p->fill == YES)
				{
				...
				if (p->paramId == TOPOLOGY_NCL_FIXED ||
                    ...
                    if (CopyToTreeFromPolyTree(tree, userTree[modelParams[p->relParts[0]].topologyFix]) == ERROR)
						{
...

{
	...
	*isApplic2 = YES;
	...
	if (whichParam == P_TRATIO)
			...
			if (!strcmp(modelParams[part1].brlensPr, "Clock") && !strcmp(modelParams[part2].brlensPr, "Clock"))
				...
				if (strcmp(modelParams[part1].clockPr, modelParams[part2].clockPr))
					...
					if (strcmp(modelParams[part1].clockPr, "Uniform") == 0)
						...
						else if (!strcmp(modelParams[part1].treeHeightPr,"Gamma"))
							{
							if (AreDoublesEqual (modelParams[part1].treeHeightGamma[0], modelParams[part2].treeHeightGamma[0], (MrBFlt) 0.00001) == NO)
								isSame = NO;
							if (AreDoublesEqual (modelParams[part1].treeHeightGamma[1], modelParams[part2].treeHeightGamma[1], (MrBFlt) 0.00001) == NO)
								isSame = NO;
							}
						}
					}
	...
	else if (whichParam == P_PSIGAMMASHAPE)
		{
		/* Check psigamma shape prior for partitions 1 and 2. */
		
		/* Check if the model is parsimony for either partition. If so, then the branch lengths cannot apply (as parsimony is very
		silly and doesn't take this information into account) and psigamma is inapplicable. */
		if (!strcmp(modelParams[part1].parsModel, "Yes"))
			*isApplic1 = NO; 
		if (!strcmp(modelParams[part2].parsModel, "Yes"))
			*isApplic2 = NO; 
		...
		if (!strcmp(modelParams[part1].psiGammaPr,"Uniform") && !strcmp(modelParams[part2].psiGammaPr,"Uniform"))
			{
			if (AreDoublesEqual (modelParams[part1].psiGammaUni[0], modelParams[part2].psiGammaUni[0], (MrBFlt) 0.00001) == NO)
				isSame = NO;
			if (AreDoublesEqual (modelParams[part1].psiGammaUni[1], modelParams[part2].psiGammaUni[1], (MrBFlt) 0.00001) == NO)
				isSame = NO;
			}
		else if (!strcmp(modelParams[part1].psiGammaPr,"Exponential") && !strcmp(modelParams[part2].psiGammaPr,"Exponential"))
			{
			if (AreDoublesEqual (modelParams[part1].psiGammaExp, modelParams[part2].psiGammaExp, (MrBFlt) 0.00001) == NO)
				isSame = NO;
			}
		else if (!strcmp(modelParams[part1].psiGammaPr,"Fixed") && !strcmp(modelParams[part2].psiGammaPr,"Fixed"))
			{
			if (AreDoublesEqual (modelParams[part1].psiGammaFix, modelParams[part2].psiGammaFix, (MrBFlt) 0.00001) == NO)
				isSame = NO;
			}
		else
			isSame = NO;
		
		/* Check to see if psigamma shape is inapplicable for either partition. */
		if ((*isApplic1) == NO || (*isApplic2) == NO)
			isSame = NO; 	
		}
		...
		if (IsModelSame (P_PSIGAMMASHAPE, part1, part2, &temp1, &temp2) == NO)
			isSame = NO;
		...
		/* Check to see if bm variance is inapplicable for either partition. */
		if ((*isApplic1) == NO || (*isApplic2) == NO)
			isSame = NO; 	
		}
	else if (whichParam == P_BMBRANCHRATES)
		{
		/* Check BM relaxed clock branch rates for partitions 1 and 2. */
	
		/* Check if the model is parsimony for either partition. If so, then branch lengths do not apply (as parsimony is very
		silly and doesn't take this information into account) and bm relaxed clock branch rates are inapplicable. */
		if (!strcmp(modelParams[part1].parsModel, "Yes"))
			*isApplic1 = NO; 
		if (!strcmp(modelParams[part2].parsModel, "Yes"))
			*isApplic2 = NO; 
		...
		/* Check if the model is parsimony for either partition. If so, then the branch lengths cannot apply (as parsimony is very
		silly and doesn't take this information into account) and ibr gamma shape is inapplicable. */
		if (!strcmp(modelParams[part1].parsModel, "Yes"))
			*isApplic1 = NO; 
		if (!strcmp(modelParams[part2].parsModel, "Yes"))
			*isApplic2 = NO; 
	...
	else if (whichParam == P_IBRBRANCHRATES)
		{
		/* Check IBR relaxed clock branch rates for partitions 1 and 2. */
...

{
	...
	int			    isBaseRateNeeded, c, i, j, k, n, n1, n2, *isPartTouched,
				    numRelParts, nRelParts, areAllPartsParsimony, nClockBrlens, nRelaxedBrlens, nCalibratedBrlens;
...
#	endif
	...
	for (k=0; k<numParams; k++)
        ...
        /* do things differently for a base rate parameter */
        if (j == NUM_LINKED)     /* no match with regular parameters */
            {
            for (i=0; i<numCurrentDivisions; i++)
				{
		...
		if (j == P_TRATIO)
            ...
            p->paramTypeName = "Base rate of evolution";
			strcpy (p->name, "Baserate");   /* applies to all partitions, even those that do have
                                               a rate multiplier on top */
            ...
            p->paramTypeName = "Rate of rate-multiplying compound poisson process (CPP)";
            ...
            p->paramTypeName = "Events of Compound Poisson Process";
			strcpy (p->name, "CppEvents");
			strcat (p->name, partString);
			
			/* find the parameter x prior type */
			p->paramId = CPPEVENTS;
			
			/* should we print the events? */
			for (i=0; i<p->nRelParts; i++)
				if (!strcmp(modelParams[p->relParts[i]].treeFormat,"Events"))
					break;
			if (i < p->nRelParts)
				p->printParam = YES;
			else
		...
		else if (j == P_PSIGAMMASHAPE)
			{
			/* Set up psigammashape *****************************************************************************************/
			p->paramType = P_PSIGAMMASHAPE;
			p->nValues = 1;
			p->nSubValues = 0;
            p->min = 0.0;
            ...
            p->paramTypeName = "Shape of psigamma distribution of CPP rate multiplier";
			strcpy (p->name, "Alpha_psi");
			strcat (p->name, partString);
					
			/* find the parameter x prior type */
			if (!strcmp(mp->psiGammaPr,"Uniform"))
				p->paramId = PSIGAMMASHAPE_UNI;
			else if (!strcmp(mp->psiGammaPr,"Exponential"))
				p->paramId = PSIGAMMASHAPE_EXP;
			else
				p->paramId = PSIGAMMASHAPE_FIX;
			
			if (p->paramId != PSIGAMMASHAPE_FIX)
				p->printParam = YES;
			SafeStrcat (&p->paramHeader, "alpha_psi");
			SafeStrcat (&p->paramHeader, partString);
			}
		else if (j == P_NU)
			{
			...
			{
			/* Set up bm relaxed clock rates parameter *****************************************************************************************/
			...
			{
			/* Set up ibr relaxed clock rates parameter *****************************************************************************************/
...
{
	int			i, j, k, *nEvents=NULL, *nEventsP=NULL, nLongsNeeded;
	MrBFlt		*bmBranchRate=NULL, *bmBranchRateP=NULL, *ibrBranchRate=NULL, *ibrBranchRateP=NULL,
	...
	if (param->paramType == P_CPPEVENTS)
		...
		{
		/* find the right branch rate set */
		...
		if (param->paramType == P_CPPEVENTS)
			...
			if (nEvents[p->index] != nEventsP[pp->index])
				...
				if (nEventsP[pp->index] == 0)
					...
					{
					position[p->index] = (MrBFlt *) realloc ((void *) position[p->index], nEventsP[pp->index]*sizeof(MrBFlt));
					rateMult[p->index] = (MrBFlt *) realloc ((void *) rateMult[p->index], nEventsP[pp->index]*sizeof(MrBFlt));
					}
			...
			{
			bmBranchRate[p->index] = bmBranchRateP[pp->index];
			}
		else if (param->paramType == P_IBRBRANCHRATES)
			{
			ibrBranchRate[p->index] = ibrBranchRateP[pp->index];
			}
		}
    ...
    if (param->paramType == P_CPPEVENTS)
        ...
        {
        if (UpdateBmEvolLengths (param, t, chn) == ERROR)
            return (ERROR);
        }
    else if (param->paramType == P_IBRBRANCHRATES)
        {
        if (UpdateIbrEvolLengths (param, t, chn) == ERROR)
            return (ERROR);
        }
...

{
	...
	/* Move_ExtSPR1 */
	mt = &moveTypes[i++];
	mt->name = "Extending SPR version 1";
	mt->shortName = "eSPR1";
    mt->subParams = YES;
	mt->tuningName[0] = "Extension probability";
	mt->shortTuningName[0] = "p_ext";
	...
	/* Move_TreeHeight */
	mt = &moveTypes[i++];
	mt->name = "Tree height slider";
	mt->shortName = "THSlider";
	mt->tuningName[0] = "Sliding window size";
	mt->shortTuningName[0] = "delta";
	mt->applicableTo[0] = BRLENS_CLOCK_UNI;
	mt->applicableTo[1] = BRLENS_CLOCK_COAL;
	...
	/* Move_TreeHeightM */
	mt = &moveTypes[i++];
	mt->name = "Tree height multiplier";
	mt->shortName = "THMult";
	mt->tuningName[0] = "Multiplier tuning parameter";
	mt->shortTuningName[0] = "lambda";
	mt->applicableTo[0] = BRLENS_CLOCK_UNI;
	...
	/* Move_CPPPsiGammaAlpha */
	mt = &moveTypes[i++];
	mt->name = "Multiplier";
	mt->shortName = "Multiplier";
	mt->tuningName[0] = "Multiplier tuning parameter";
	mt->shortTuningName[0] = "lambda";
	mt->applicableTo[0] = PSIGAMMASHAPE_EXP;
	mt->applicableTo[1] = PSIGAMMASHAPE_UNI;
	mt->nApplicable = 2;
	mt->moveFxn = &Move_CPPPsiGammaAlpha;
	mt->relProposalProb = 1.0;
	mt->numTuningParams = 1;
	mt->tuningParam[0] = 2.0 * log (1.1);  /* lambda */
	mt->minimum[0] = 0.00001;
	...
	mt->tuningName[0] = "Multiplier tuning parameter";
	mt->shortTuningName[0] = "lambda";
	mt->applicableTo[0] = BMBRANCHRATES;
	mt->nApplicable = 1;
	...
	mt->tuningName[0] = "Multiplier tuning parameter";
	mt->shortTuningName[0] = "lambda";
	mt->applicableTo[0] = IBRBRANCHRATES;
	mt->nApplicable = 1;
...

{
	...
	for (j=0; j<NUM_LINKED; j++)
		...
		if (j == P_TRATIO)
			...
			{
			MrBayesPrint ("%s      Psigammashape  ", spacer);
			}
		else if (j == P_CPPEVENTS)
			{
	...
	for (i=0; i<numParams; i++)
		...
		/* skip base rate parameter, if present */
		if (j == P_RATEMULT && p->nSubValues == 0)
			continue;
		...
		if (j == P_TRATIO)
			...
			if (!strcmp(mp->psiGammaPr,"Uniform"))
				...
				MrBayesPrint ("%s            Prior      = Exponential(%1.2lf)\n", spacer, mp->psiGammaExp);
			else
				MrBayesPrint ("%s            Prior      = Fixed(%1.2lf)\n", spacer, mp->psiGammaFix);
			}
		else if (j == P_CPPEVENTS)
			{
			MrBayesPrint ("%s            Prior      = Poisson (%s) [Events]\n", spacer, modelSettings[p->relParts[0]].cppRate->name);
			MrBayesPrint ("%s                         Gamma (%s,Psi(%s)) [Rate multipliers]\n", spacer, modelSettings[p->relParts[0]].psiGamma->name, modelSettings[p->relParts[0]].psiGamma->name);
			}
		else if (j == P_NU)
			{
...
/* UpdateBmEvolLengths: update branch lengths for bm model */
int UpdateBmEvolLengths(Param *param, Tree *t, int chain)
{
    int         i;
    MrBFlt      *bmRate, *brlens;
    TreeNode    *p;
...
/* UpdateIbrEvolLengths: update branch lengths for ibr model */
int UpdateIbrEvolLengths(Param *param, Tree *t, int chain)
{
    int         i;
    MrBFlt      *ibrRate, *brlens;
    TreeNode    *p;
    
    ibrRate = GetParamVals (param, chain, state[chain]);
	brlens = GetParamSubVals (param, chain, state[chain]);
	for (i=0; i<t->nNodes-2; i++)
		{
		p = t->allDownPass[i];
