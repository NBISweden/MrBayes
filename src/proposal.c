...
#   endif
    ...
    while (p->anc->isLocked == YES || p->anc->anc->anc == NULL ||
           p->length < TIME_MIN || a->length < TIME_MIN || b->length < TIME_MIN);
    /* skip constraints, siblings of root (and root), and ancestral fossils (brl=0) in fbd tree */
...
{
    /* Change branch lengths and topology using SPR-type move weighted according to parsimony scores. */
    
    int         i, j, k, n, division, topologyHasChanged, isVPriorExp,
                moveInRoot, nNeighbor, nRoot, nCrown, iA, jC;
...
#   endif
    ...
    if (c->nodeDepth > v->nodeDepth)
        newBrlen = d->nodeDepth - c->nodeDepth;
    else
        newBrlen = d->nodeDepth - v->nodeDepth;

    if (newBrlen < 2.0 * minV)
        {
        abortMove = YES;
        free (nSitesOfPat);
        return (NO_ERROR);
    ...
    if (v->length > 0.0)
        {
        /* adjust lengths */
        newPos = RandomNumber(seed) * (newBrlen - 2.0 * minV);
...
{
    /* Change branch lengths and topology (potentially) using TBR-type move biased according to parsimony scores. */
