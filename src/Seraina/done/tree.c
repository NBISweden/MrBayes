...
const char* const svnRevisionTreeC="$Rev: 555 $";   /* Revision keyword which is expended/updated by svn on each commit/update*/
...
int IsTreeConsistent (Param *param, int chain, int state)
{
    ...
    if (tree->isRooted == NO)  //SK changed to account for rooted, non-clock trees (no further testing necessary)
        {
    ...
    else if (tree->isClock == NO) //SK: if tree is unrooted AND non-clock (let's get out of here)
        return YES;
