...
const char* const svnRevisionSumpC="$Rev: 559 $";   /* Revision keyword which is expended/updated by svn on each commit/update*/
...
int PrintParamStats (char *fileName, char **headerNames, int nHeaders, ParameterSample *parameterSamples, int nRuns, int nSamples)
{
    ...
    /* allocate and set nSamples */
    sampleCounts = (int *) SafeCalloc (nRuns, sizeof(int));
    validSampleCounts = (int *) SafeCalloc (nRuns, sizeof(int)); /* for parameters where we want to remove NAs first */
	...
	/* allocate a separate matrix for parameters that need NAs removed */ //SK
	...
	for (i=0; i<nHeaders; i++)
		...
		if (IsSame (temp, "lnL") == SAME)
			continue;
		/* removing NA values for chosen parameters */ 
		if (IsSame (temp, "rootpi(0)") == SAME || IsSame (temp, "rootpi(1)") == SAME) //SK: currently only doing NA removal for root freqs. Can be extended in the future.
			{
			for (k=0; k<nRuns; k++)
				{
