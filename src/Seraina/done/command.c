...
const char* const svnRevisionCommandC="$Rev: 565 $";   /* Revision keyword which is expended/updated by svn on each commit/update*/
...
CmdType			commands[] =
            ...
            { 22,            "Link",  NO,            DoLink, 27,  {55,56,57,58,59,60,61,62,63,72,73,74,75,76,105,118,193,194,195,196,197,242,243,252,253,255,256},
                4,               "Links parameters across character partitions",  IN_CMD, SHOW },
            { 23,             "Log",  NO,             DoLog,  5,                                                                                 {85,86,87,88,89},         4,                               "Logs screen output to a file",  IN_CMD, SHOW },
            { 24,            "Lset",  NO,            DoLset, 17,                                         {28,29,30,31,32,33,34,40,51,52,53,90,91,131,188,189,270},        4,                "Sets the parameters of the likelihood model",  IN_CMD, SHOW },
...

{
    ...
    */
    ...
    if (isTaxsetDef == NO)
		{
		MrBayesPrint ("%s   Taxon labels must be specified before a tree could be read in\n", spacer);
		return (ERROR);
		}
	if (inTreesBlock == NO)
		{
	...
	if (expecting == Expecting(PARAMETER))
		...
		if (foundAmpersand == YES)
			...
			if (strcmp(tkn,"E") == 0)
                ...
                {
                t->isRooted = YES;
                t->isClock = YES;   /* assume clock if rooted */ //SK: remove this in case I want to read in user trees for directional model! Does that break anything else?
...

{
	...
	if (!strcmp(helpTkn, "Begin"))
		...
		MrBayesPrint ("                    prset:                                                       \n");
		MrBayesPrint ("                                                                                 \n");
		MrBayesPrint ("                       prset applyto=(1,2) statefreqpr=fixed(equal)              \n");
		MrBayesPrint ("                                                                                 \n");
		MrBayesPrint ("                       prset applyto=(3) statefreqpr=dirichlet(1,1,1,1)          \n");
		MrBayesPrint ("                                                                                 \n");
...

{
	...
	PARAM   (270, "StatefreqModel", DoLsetParm,         "Stationary|Directional|Mixed|\0"); //SK
	PARAM   (271, "Rootfreqpr",     DoPrsetParm,        "Dirichlet|Fixed|\0"); //SK
    
	/* NOTE: If a change is made to the parameter table, make certain you
	         change the number of elements (now 272) in NUMPARAMS at the top of this file. */
    /* CmdType commands[] */
}
