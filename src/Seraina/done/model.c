...
const char* const svnRevisionModelC="$Rev: 555 $";   /* Revision keyword which is expended/updated by svn on each commit/update*/
...
{
	int			i, j, k, n, nOfParams, nOfTrees, isRooted, numSubParamPtrs, allStationaryModel;
	Param		*p, *q;
	...
	for (k=0; k<numParams; k++)
		...
		if (p->paramType == P_BRLENS)
			{
			/* find type of tree */
            allStationaryModel = 1; //SK: check if all relevant partitions follow a stationary model; if not, use rooted trees!
            for (i=0; i<p->nRelParts; i++) 
                {
                if (strcmp(modelParams[p->relParts[i]].statefreqModel, "Stationary"))
                    {
                    allStationaryModel = 0;
                    break;
                    }
                }
            
			if (!strcmp(modelParams[p->relParts[0]].brlensPr,"Clock") || !allStationaryModel)
				isRooted = YES;
			else
				isRooted = NO;
	...
	for (chn=fromChain; chn<toChain; chn++)
		...
		for (k=0; k<numParams; k++)
			...
			if (p->paramType == P_TRATIO)
                ...
                /* fill for the directional model of evolution for state frequencies */ //SK
	...
	if (whichParam == P_TRATIO)
		...
		if (modelSettings[part1].dataType == PROTEIN && modelSettings[part2].dataType == PROTEIN)
			...
			if (!strcmp(modelParams[part1].stateFreqPr, modelParams[part2].stateFreqPr))
				...
				} //SK: aren't here some "else isSame=NO;" missing?
...
#	endif
	...
	for (k=0; k<numParams; k++)
		...
		if (j == P_TRATIO)
            ...
            if (mp->dataType == STANDARD)
				{
				p->paramTypeName = "Symmetric dirichlet/beta distribution alpha_i parameter";
            ...
            if (strcmp(mp->statefreqModel,"Stationary"))
                {
                p->paramTypeName = "Stationary and root state frequencies";
                SafeStrcat(&p->name, "Pi_rootPi");
                }
			else
				{
			...
			if (mp->dataType == STANDARD)
                ...
                if (!strcmp(mp->statefreqModel,"Stationary")) //SK: if not directional or rj model
                    {
                ...
                else  //SK - if statefreqmodel!=stationary
                    {
                    if (mp->dataType != RESTRICTION)
                        {
			...
			if (areAllPartsParsimony == YES)
				p->paramTypeName = "Topology"; /*Why again we set it to "Topology" we did it unconditionally 1 lines before*/ //SK: good question...
...

{
	...
	mt->maximum[2] = 0.30;
	mt->parsimonyBased = YES;
	mt->level = DEVELOPER; //SK: turned move of in code //STANDARD_USER;
	...
	mt = &moveTypes[i++];
	mt->name = "Dirichlet proposal";
	mt->shortName = "DirichletDir";  //SK: remove "Dir" later, for debug...
	mt->tuningName[0] = "Dirichlet parameter";
	mt->shortTuningName[0] = "alpha";
	...
	mt = &moveTypes[i++];
	mt->name = "Sliding window";
	mt->shortName = "SliderDir"; //SK: remove "Dir" later, for debug...
	mt->tuningName[0] = "Sliding window size";
	mt->shortTuningName[0] = "delta";
	mt->applicableTo[0] = DIRPI_DIRxDIR;
	mt->applicableTo[1] = DIRPI_FIXEDxDIR;
			...
			if (ms->dataType == STANDARD)
                ...
                if (!strcmp(mp->statefreqModel, "Stationary"))
                    ...
                    if (!strcmp(mp->stateFreqPr,"Dirichlet"))
                        {
                        if (!strcmp(mp->rootFreqPr,"Dirichlet"))
                            MrBayesPrint ("%s            Prior      = Dirichlet/Dirichlet\n", spacer);
                        else
                            MrBayesPrint ("%s            Prior      = Dirichlet/Fixed\n", spacer);
                        }
                    else
                        {
                        if (!strcmp(mp->rootFreqPr,"Dirichlet"))
                            MrBayesPrint ("%s            Prior      = Fixed/Dirichlet\n", spacer);
                        else
                            MrBayesPrint ("%s            Prior      = Fixed/Fixed\n", spacer);
