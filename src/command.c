...

{
	...
	/*	MrBayesPrint ("   Growthpr      -- This parameter sets the prior on the exponential growth      \n");
		...
		MrBayesPrint ("   Sumss                                                                         \n");
	    MrBayesPrint ("                                                                                 \n");
	    MrBayesPrint ("   The command summarizes results of stepping stone analyzes. It is a tool to    \n");
	    MrBayesPrint ("   investigate correctness of obtained results, and to help find proper step     \n");
	    MrBayesPrint ("   burn-in. To get more help about stepping stone analyzes use 'help ss' command.\n");
        MrBayesPrint ("                                                                                 \n");
		MrBayesPrint ("   During stepping stone analysis, MrBayes collects sampled liklihoods in order  \n");
        MrBayesPrint ("   to estimate marginal liklihood. It also prints the sampled parameter values to\n");
		MrBayesPrint ("   one or more tab-delimited text files, one for each independent run in your    \n");
        MrBayesPrint ("   analysis. The command 'Sumss' summarize liklihood values stored in this para- \n");
        MrBayesPrint ("   meter files and calculate marginal liklihood estimates. The names of the files\n");
        MrBayesPrint ("   that are summarized are exactly the same as the one used in 'sump' comman. In \n");
        MrBayesPrint ("   fact the file names is a shared setting for 'sump' and 'sumss' commands,i.e if\n");
        MrBayesPrint ("   you change the setting in one of the command, it would change the setting in  \n");
        MrBayesPrint ("   the other one as well.                                                        \n");
        MrBayesPrint ("                                                                                 \n");
        MrBayesPrint ("   When you invoke the 'Sumss' command, three items are output: (1) 'Step contri-\n");
        MrBayesPrint ("   bution table' - summarizes estimates of the marginal likelihood and contribu- \n");
        MrBayesPrint ("   tion of each step to it; (2) 'Step plot' - plot of the likelihood values for a\n");
        MrBayesPrint ("   given step or burn-in phase; (3) 'Joined plot' - summmarizes at the same plot \n");
        MrBayesPrint ("   sampling for all steps; (see below).                                          \n");
	    MrBayesPrint ("                                                                                 \n");
        MrBayesPrint ("   'Step contribution table'                                                     \n");
        MrBayesPrint ("   The printed table is similar to the one which is output to .ss file. The main \n");
        ...
        MrBayesPrint ("   analizes. Which step is printed here is defined by 'Steptoplot' setting. The  \n");
        MrBayesPrint ("   plot could be used to inspect if chosen step burn-in is good enough for a     \n");
        MrBayesPrint ("   given step. It could also be used to check if burn-in phase has converged.    \n");
        MrBayesPrint ("   Note that amount of discarded samples is controled by 'Discardfrac' settings, \n");
        MrBayesPrint ("   and not by ordinary burn-in settings.                                         \n");
	    MrBayesPrint ("                                                                                 \n");
        MrBayesPrint ("   'Joined plot'                                                                 \n");
	    MrBayesPrint ("   Different steps sample from different power posterior distributions. When we  \n");
        MrBayesPrint ("   switch from sampling one step to another, it takes some number of generations \n");
        MrBayesPrint ("   before we settle at the stationary for a new distribution. The generations in \n");
        MrBayesPrint ("   transition have a temperature lag and they need to be discarded as burn-in    \n");
        MrBayesPrint ("   within steps. The good news is that you can chose burn-in within steps with-  \n");
        MrBayesPrint ("   out rerunning the whole analyzes.  The 'Joined plot' helps to facilitate the  \n");
        MrBayesPrint ("   decision on choosing right burn-in. The plot summarizes samples across all    \n");
        MrBayesPrint ("   steps and gives quick overview of the whole analyzes.                         \n");
        MrBayesPrint ("   The description of the plot:                                                  \n");
        MrBayesPrint ("   Each step has the same number N of samples taken. We number each sample 1 to N\n");
        MrBayesPrint ("   within steps according to the order the samples are taken. First sample which \n"); 
        MrBayesPrint ("   is taken after we start sampling a step is numbered 1, and the last sample    \n");
        MrBayesPrint ("   taken is numbered N. For each number i in [1,..., N] we sum up log likelihoods\n");
        MrBayesPrint ("   for all samples numbered i across all steps. The 'Joined plot' is a graph of  \n");
        MrBayesPrint ("   step numbers versus normalized sums we get in the procedure describe above.   \n");
        MrBayesPrint ("   Burn-in, which is controled by 'Relburnin','Burnin' and 'Burninfrac' settings,\n");
        MrBayesPrint ("   is not printed.                                                               \n");
        MrBayesPrint ("   Ideally, after you discard appropriate burn-in, you should get a graph looking\n");
        MrBayesPrint ("   as a white noise. If you have either rising or descending tendency in the     \n");
        MrBayesPrint ("   beginning of the graph, you should increase burn-in within steps. If you have \n");
        MrBayesPrint ("   rising or descending tendency across the whole graph then initial burn-in,    \n");
        MrBayesPrint ("   that you had before starting sampling steps, was not sufficient. In that case \n");
        MrBayesPrint ("   all steps through which mcmc was burning in need to be re-sampled with appro- \n");
        MrBayesPrint ("   priate initial burn-in.                                                       \n");
        MrBayesPrint ("   To make easer observation of tendencies in the plotted graph you can choose   \n");
        MrBayesPrint ("   different level of curve smoothing. Curve smoothing equal to k, means that    \n");
        MrBayesPrint ("   for each step i we take average over obtained sum for the step i and k neigh- \n");
        MrBayesPrint ("   bor samples in both direction, i.e. new value for step i is an average over   \n");
        MrBayesPrint ("   values of steps [i-k,...,i+k].                                                \n");
	    MrBayesPrint ("                                                                                 \n");
	    MrBayesPrint ("                                                                                 \n");
		MrBayesPrint ("   Options:                                                                      \n");
	    MrBayesPrint ("                                                                                 \n");
		MrBayesPrint ("   Allruns      -- If set to 'Yes', it makes all runs to be printed on the same  \n");
		MrBayesPrint ("                   graph when drowing joined and step plots. If set to 'No', each\n");
        MrBayesPrint ("                   run is printed on a separat plot.                             \n");
		MrBayesPrint ("   Askmore      -- Long analyzes may produce huge .p files. Reading in them may  \n");
        MrBayesPrint ("                   take several minutes. If you want to investigate different    \n");
        MrBayesPrint ("                   aspects of your analyzes, it could be very inconvenient to    \n");
        MrBayesPrint ("                   wait for several minutes each time you want to get a new sum- \n");
        MrBayesPrint ("                   mery for different settings. If you set 'Askmore’ to ‘YES',   \n");
        MrBayesPrint ("                   sumss will read .p files only once. After making ordinary sum-\n");
        MrBayesPrint ("                   mery, it would interactivaly ask you to print more tables and \n");
        MrBayesPrint ("                   plots for different settings of ‘Burnin’ or ‘Smoothing'(see   \n");
        MrBayesPrint ("                   below).                                                       \n");
		MrBayesPrint ("   Burnin       -- Determines the number of samples (not generations) that will  \n");
		MrBayesPrint ("                   be discarded when summary statistics are calculated. The      \n");
		MrBayesPrint ("                   value of this option is only applicable when 'Relburnin' is   \n");
		...
		MrBayesPrint ("                   'Step plot' is printed. It is similar to 'Burninfrac' setting,\n");
		MrBayesPrint ("                   but unlike 'Burninfrac', it is used only for better visuali-  \n");
        MrBayesPrint ("                   zation of 'Step plot'. It facilitates analyzes of initial burn\n");
        MrBayesPrint ("                   in convergence and step's temperature lag, but it has no      \n");
        MrBayesPrint ("                   effect on the amount of sample discarded during marginal like-\n");
        MrBayesPrint ("                   lihood computation.                                           \n");
		MrBayesPrint ("   Filename     -- The name of the file to be summarized. This is the base of the\n");
		MrBayesPrint ("                   file name to which endings are added according to the current \n");
		MrBayesPrint ("                   setting of the 'Nruns' parameter. If 'Nruns' is 1, then only  \n");
		MrBayesPrint ("                   '.p' is added to the file name. Otherwise, the endings will   \n");
		MrBayesPrint ("                   be '.run1.p', '.run2.p', etc. Note that 'Filename' setting is \n");
        MrBayesPrint ("                   shared with 'sump' command.                                   \n");
		MrBayesPrint ("   Nruns        -- Determines how many '.p' files from independent analyses that \n");
		MrBayesPrint ("                   will be summarized. If Nruns > 1 then the names of the files  \n");
		MrBayesPrint ("                   are derived from 'Filename' by adding '.run1.p', '.run2.p',   \n");
		MrBayesPrint ("                   etc. If Nruns=1, then the single file name is obtained by     \n");
        ...
        MrBayesPrint ("                   settings:'Relburnin', 'Burnin' and 'Burninfrac' are shared    \n");
		MrBayesPrint ("                   across the 'sumt', 'sump', 'sumss' and 'mcmc' commands.       \n");
		MrBayesPrint ("   Steptoplot   -- Defines which step will be printed at the 'Step plot'.If value\n");
        MrBayesPrint ("                   is set to 0, then burn-in phase will be printed.              \n");
		MrBayesPrint ("   Smoothing    -- Determines smoothing of 'Joined plot' (see above). Value equal\n");
        MrBayesPrint ("                   to 0 effectivly does no smoothing.                            \n");
	    MrBayesPrint ("                                                                                 \n");
		MrBayesPrint ("   Current settings:                                                             \n");
	    MrBayesPrint ("                                                                                 \n");
		MrBayesPrint ("   Parameter       Options                  Current Setting                      \n");
