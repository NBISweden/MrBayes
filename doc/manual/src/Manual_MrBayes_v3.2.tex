...
%%%%% JN Nov-29: The description here is dependent on a file "cynmix-run.nex" that are no longer present.
%%%%% JN: I suggest changing BOTH here and in the cynmix.nex file (coomenting-out the entire mb block)
...
Note also that each command ends with a semicolon (\texttt{;}).
...
convenient to use predefined character sets as given in the mrbayes block. The final step is to tell 
MrBayes that we want to work with this partitioning of the data instead of the default partitioning. 
We do this using the \texttt{set} command:
...
The \texttt{end} statement closees the MrBayes block.

When we read this block into MrBayes, we will get a partitioned model with the first character division 
being morphology, the second division being the COI gene, etc. Exit your text editor, launch MrBayes and 
type \textbf{execute cynmix.nex} to read in your data and set up the partitioning scheme.
