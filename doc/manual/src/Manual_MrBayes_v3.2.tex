...
\tb{3.1}. At the \texttt{MrBayes >} prompt, type \tb{mcmc ngen=20000 samplefreq=100}\\
\tb{printfreq=100 diagnfreq=1000}. This will start the Markov chain Monte Carlo simulation. It will
...
\tb{4.2}. Summarize the trees using the same burn-in as the \texttt{mcmc} command by typing
\tb{sumt}.  The program will output a cladogram with the posterior probabilities for each split and
a phylogram with mean branch lengths. Both trees will also be printed to a file that can be read by
FigTree \citep{rambaut12} and other tree-drawing programs, such as TreeView \citep{page96} and
Mesquite \citep{maddison06}.
...
%\centering
%\begin{BVerbatim}[baselinestretch=0.8,fontsize=\footnotesize]
\begin{Verbatim}[baselinestretch=0.8,xleftmargin=15mm,fontsize=\footnotesize]
Parameter       Options               Current Setting
--------------------------------------------------------
Ngen            <number>              1000000
Nruns           <number>              2
...
%\begin{figure}[H]
%\centering
%\begin{BVerbatim}[baselinestretch=0.8,xleftmargin=15mm,fontsize=\footnotesize]
%Key to taxon bipartitions (saved to file "primates.nex.parts"):
%
%ID -- Partition
%------------------
...
the file with the ending \texttt{.con.tre} in a format that can be understood by the program
FigTree.  When you open the file in FigTree, you will have access to a range of options for
...
be excluded (\texttt{morph\_excluded}). Then these characters are ordered and excluded with the
following two lines:
...
%%%% JN: tor  2 jul 2020 21:44:19 Need to verify that commands work
...
%%%% JN: What is the limit (number X) for reading trees? Need to specify that below:
%%%% JN: "You can read in up to $?$ trees."
