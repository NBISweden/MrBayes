...
\author{Fredrik Ronquist, John Huelsenbeck, Maxim Teslenko, Johan Nylander}
...
%\subsubsection{Metropolis coupling}
...
checkpoint file every \ttt{Checkfreq} generations, by default every $2000$ generations. The
...
$$
BF_{12} = \frac{Pr(Data|Hypothesis_{1})}{Pr(Ddata|Hypothesis_{2})}
$$
...
default prior used in MrBayes is an exponential distribution with mean set to $1.0$ (\ttt{prset
shapepr=exponential(1.0)}). The sampled values of the shape parameter are found under the column
...
% Comment JN: This subsection needs to be expanded
