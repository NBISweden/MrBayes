...
\tb{3.1}. At the \ttt{MrBayes >} prompt, type \tb{mcmc ngen=20000 samplefreq=100}\\ \tb{printfreq=100
...
%% PV: removed white spaces at the left to reduce the effect of long lines extending beyond textwidth
...
%% PV: If we make some room, we can afford footnotesize here
\begin{figure}[h]\singlespacing\footnotesize
\begin{verbatim}
...
% PV: ... and we can afford scriptsize here also
\begin{figure}[h]\singlespacing\scriptsize
\begin{verbatim}
                                         95% HPD Interval
                                       -------------------
...
%% PV: And I need to reduce the line length here
\begin{singlespacing}\small
\begin{verbatim}
...
\begin{wrapfigure}{r}{0.4\textwidth}\singlespacing\footnotesize
\vspace{-18mm}
\begin{verbatim}
...
The output from the \texttt{ss} command will include the following tables for the first model (upper table) and second model (lower table):~\rotatebox[origin=l]{270}{$\Rsh$}

\begin{wrapfigure}{r}{0.45\textwidth}\singlespacing\footnotesize
  \vspace{-7mm}
\begin{verbatim}
...
\begin{wrapfigure}{r}{0.42\textwidth}\singlespacing\footnotesize
  \vspace{-1cm}
\begin{verbatim}
   Run     Arithmetic   Harmonic 
              mean        mean
...
MCMC samples obtained after the burn-in.~$\rightarrow$
...
Again, we focus on the table summarizing the means of the likelihoods of the MCMC samples: $\rightarrow$
...
command. It should look something like the figure in page~\pageref{phylomediannode}:

% PV: We can afford footnotesize here
\begin{figure}[h]\singlespacing\footnotesize\label{phylomediannode}
\begin{verbatim}
...
\begin{wrapfigure}{r}{0.32\textwidth}\singlespacing\footnotesize
  \vspace{-10mm}
\begin{verbatim}
     Run      Marginal 
           likelihood (ln)
...
analysis is completed. Here are the estimates from our run:~$\rightarrow$
...
We got the following estimates of the marginal model likelihoods:~$\rightarrow$
...
%% PV: (optional) If the manual is printed, including the page will help to find the table faster
...
page~\pageref{tablebftable}). If you are familiar with likelihood ratio tests, it may seem 
...
In this analysis, we will set up a relaxed clock model for the same data set, \texttt{primates.nex},
...
\texttt{set autoclose=yes nowarnings=yes} for allowing execution without being prompted for answering
\texttt{yes/no} between steps):

%% PV: Need to save a line here...
\begin{singlespacing}\small
\begin{verbatim}
MrBayes > set autoclose=yes nowarnings=yes
MrBayes > execute primates.nex
...
The \texttt{constraint} command first gives the name of the constraint (`ingroup'), then the type of
...
Thorne--Kishino 2002 (TK02) \citep{thorne02}, the compound poisson process (CPP) \citep{huelsenbeck00a}, 
the white noise (WN) \citep{lepage07}, the independent gamma rates (IGR), and independent lognormal 
(ILN) \citep{drummond06} models. We note here that, since version 3.2.8, the name `WN' is used to 
refer to `IGR' in old versions, while `IGR' is used to refer to a different model. In the new IGR model, 
the rates are i.i.d. gamma distributions across branches and the variance does not dependent on branch lengths
as in the WN model. The ILN model differs from IGR in that the rate variation follows i.i.d. lognormal 
...
the rate at which the variance of the effective branch length increases over time, \texttt{WNvarpr}.
...
%% PV: complex figure arrangement starting here. We need to reduce and rearrange some paragraphs
%% in order to be able to display the figure correctly. We will split the figure also in two parts.
If our rooting assumption is correct, then there must have been some changes in
the evolutionary rate close to the root. Specifically, we should see a deceleration in the
\textit{Lemur} and \textit{Tarsius} lineages, or an acceleration in their sister lineages.
...
We can look for this pattern in the summary statistics for the branch and node parameters, output by 
\texttt{sumt} before the trees.~$\rightarrow$

The table is preceded by a key to the taxon bipartitions, which is critical to
interpreting where the different parameters belong. Now let us find the ID of the partitions 
corresponding to the \textit{Lemur} and \textit{Tarsius} lineages, and their sister lineages. 
     
\begin{wrapfigure}{r}{0.42\textwidth}\singlespacing\footnotesize
\vspace{-16mm}
\begin{verbatim}
  Key to taxon bipartitions 
      (saved to 
    "primates.nex.parts"):
...
We first identify the taxon ID of \textit{Lemur} and find that it is 2. Now we 
find the partition ID that has a single star in the second position; it is the
bipartition that separates \textit{Lemur catta} from all other taxa. Not surprisingly, the
bipartition ID is 2. The sister lineage of \textit{L. catta} includes all taxa except itself.~$\rightarrow$

To find the corresponding bipartition ID, we need to find the ID that has a partition 
descriptor with a single dot in the second position; all other positions should be filled
...
%% PV: Widow fixed. Clearing some repetition also. This page must end there so we need to place a:
\clearpage
...
sister lineage to \textit{T. syrichta} with bipartition ID 16, which would be in
accordance with our predictions.

To help you visualize parameters such as branch rates inferred by a relaxed clock model, \texttt{sumt} 
produces a \texttt{.con.tre} file that contains summaries of all branch and node
parameter samples in a format that the program FigTree can display. This allows you, for
...
\begin{wrapfigure}{r}{0.34\textwidth}\singlespacing\footnotesize %\small
\vspace{-25mm}
\begin{verbatim}
      Run      Marginal 
           likelihood (ln)
...
first (upper table), followed by the strict clock model (lower table):~$\rightarrow$
...
%% PV: Tabula-rasa to the line but keeping the next section in this page
\vspace{8mm}   
...
\begin{singlespacing}\small
\begin{verbatim}
> partition ancstates = 6: 1, 2-166, COI, EF1a, LWRh, 28S
> set partition=ancstates
> lset applyto=(2) rates=gamma
> lset applyto=(3,4,5,6) rates=invgamma nst=6
