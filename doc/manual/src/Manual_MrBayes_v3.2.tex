...
file names, etc are also given in \ttt{typewriter font}. Furthermore, a ``\ttt{MrBayes >}'' will
indicate the prompt in MrBayes, while the ``\ttt{\$}'' indicates your computer prompt (inside your
shell/Terminal).
...
\begin{singlespacing}
\footnotesize
Ronquist, F., J. P. Huelsenbeck, M. Teslenko, C, Zhang, and J. A. A Nylander (2020) MrBayes version 3.2 manual:
...
For instance, \ttt{length[14]} is the length of the branch corresponding to partition ID 14. As we
...
characters. For MrBayes to calculate the probability of the data correctly, we need 
 to inform it of this ascertainment (coding) bias. By default, MrBayes assumes that standard 
...
is \texttt{variable} for standard (morphology) data. Now, \tb{showmodel} should produce this table:
...
%%%% JN: TODO: Need to place this table here
...
%%%% JN: TODO: Fix the placement of this figure without wrapfigure
...
bipartition ID is 2. The sister lineage of \textit{L. catta} includes all taxa except itself.
