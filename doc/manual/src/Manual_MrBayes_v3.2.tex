...
the DATA block of the Nexus file by outputting some information about the file read in.
...
This would be achieved by typing \ttt{prset statefreqpr=fixed(equal)}.
...
is set to yes and that \ttt{Diagnfreq} is set to some reasonable value. The default value of 5000
is more appropriate for a larger analysis, so change the setting so that we compute diagnostics
every 1000th generation instead by typing \tb{mcmcp diagnfreq=1000}.
...
programs like PAUP* and TreeView. The root of the \ttt{.p} and \ttt{.t} file names can be altered
using the \ttt{Filename} setting.
...
MCMC. The ESS (Effective Sample Size) is the (estimated) effective number of independent draws from
the posterior distribution that the MCMC managed to sample. A small ESS ($<100$) would indicate
poor mixing behaviour, suggesting that you need to run longer simulations. The Potential Scale
Reduction Factor (PSRF), is based on comparing the estimated between-chain variance with the
...
the GTR model, and \ttt{gtrsubmodel[121121]} to the HKY model.
...
$$
where $\theta$ is the Bayes factor.
...
most popular tools are Tracer \citep{rambaut07} and AWTY \citep{nylander07}. Among other things,
they provide nice graphical representations of the output from MCMC analyses. Another set of tools
is provided by the CODA \citep{plummer06} and RWTY \citep{warren17} packages for R
\citep{rcoreteam18}.
