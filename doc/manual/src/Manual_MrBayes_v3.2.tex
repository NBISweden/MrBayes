...
%%%% JN: far (around line #2194, Feb 2019).
...
This ratio is known as the Bayes factor. We will use the data in the file \ttt{primates.nex} for
the analysis.
...
evidence \citep{kass95}. Thus, the harmonic mean estimator indicates that we have very strong
evidence in favor of human and chimp being each others closest relatives, with Gorilla and other
primates being more distant.
...
is the number of generations per step left --- after a burnin of (the length of) one step is removed.
...
models, about 9 log likelihood units. Thus, we conclude that the better model (humans and chimps
...
%%%% JN: It might be worth mentioning that for all difficult MCMC scenarios, it might be worth
%%%% JN: repeating the analysis, and using more resources, and for ss, to repeat using
%%%% JN: fromprior=yes 
%%%% JN: Done this far Mon 22 jul 2019 17:28:33
\vspace{10 pt}
...
%%%% JN: be consistent: $B_{10}$ or $BF_{12}$ 
...
\begin{longtable}[]{@{}lll@{}}
\toprule
\(2\log_{e}(B_{10})\) & \(B_{10}\) & Evidence against
\(H_{0}\)\tabularnewline
\midrule
\endhead
...
\bottomrule
    \caption{Table for interpreting the Bayes factor. Based on \citep{kass95}.}
\label{table:bftable}
\end{longtable}
...
while the other is unconstrained, or one model can have $\Gamma$-shaped rate variation while the
other assumes equal rates across sites. As stated above, models need not be hierarchically nested.
An interesting property of the Bayes factor comparisons is that it can favor either the more
complex model or the simpler model, so they need not be corrected for the number of parameters in
the models being compared. Additional discussion of Bayesian model testing, with several examples,
is found in \citet{nylander04}.
...
A more typical GTR + I + $\Gamma$ model would have only little more than 200 parameters, about 1,000
