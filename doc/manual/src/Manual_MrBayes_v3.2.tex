...
%%%% JN: Have done this far tis 23 jul 2019 16:17:35
\vspace{10 pt}
{\LARGE\color{red} Please note, the following section of the manual is not revised for v.3.2.7!}
...
model \citep{kass95}. Our runs indicate that the result is going to be similar if we were to repeat
...
\begin{verbatim}
  ...
  ----------|-------------|-------------|-------------|-------------|             
        4.00e-04      3.00e-04      2.00e-04      1.00e-04      2.71e-20          
...
%%%% JN: tis 23 jul 2019 16:55:36: Ad hoc work around for bug #116.
{\color{red}
Please note, a bug in v.3.2.7a (June 2019) prevents the \ttt{sumt} command to complete. See
\url{https://github.com/NBISweden/MrBayes/issues/116}. One may, however, apply a work-around that
will allow the analysis to complete:} 

\footnotesize
To continue the example above based on the data in \ttt{finch.nex}, type \tb{quit} to quit MrBayes
after attempting the \ttt{sumt}, and continue in your shell (your Terminal window (``bash'',
applicable on Macintosh, Unix, and later versions of MS Windows), and assuming that your MrBayes
program is named \tb{mb}):

\scriptsize
\begin{singlespacing}
\begin{verbatim}
$ cat << ADHOC > finch.nex.tree31
#NEXUS
Begin data;
...
Here is an example of a simple Nexus file containing a tree block with a single tree in it:
...
presented by Kass and Raftery \citet{kass95} (see Table \ref{table:bftable}).
...
Please refer to the GitHub pages \url{https://github.com/NBISweden/MrBayes}, and especially the
files \ttt{INSTALL}, and \ttt{README} that comes with the source code, for further details.
...
\ttt{outgroup} command. For instance, if you want taxon number 7 called ``My\_taxon'' to be the
outgroup, either use \tb{outgroup 7} or \tb{outgroup My\_taxon}. MrBayes 3.2 only allows a single
taxon as the outgroup.

Before the constraints take effect, you have to invoke them by using \ttt{prset
topologypr=constraints(<comma-separated list of constraints>)}. For instance, to enforce the
constraint \ttt{my\_constraint} defined above, use \tb{prset
topologypr=constraints(my\_constraint)}.
...
%%%% JN: Old text
%The default is an exponential distribution with parameter $10$
