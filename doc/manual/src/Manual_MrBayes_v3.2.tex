...
\vspace{10 pt}
{\LARGE\color{red} Please note, the following section of the manual is not revised for v.3.2.7!}
%%%% JN: tis 23 jul 2019 16:55:36: I started on this section but encountered a bug in mb(!)
...
%%%% JN: tis 23 jul 2019 16:51:10: Here (mcmc) I get a Warning:
%%%% JN:     WARNING: You are using a coalescent model but the clock rate is fixed to 1.0.
%%%% JN:        This is likely to be incorrect unless you have set the population size prior
%%%% JN:        ('prset popsizepr') to reflect an appropriate prior on theta. 
%%%% JN:     Do you want to continue with the run regardless? (yes/no):
%%%% JN: Answering 'yes' to the question ends up in an infinite loop (asking again, and again...)
%%%% JN: The command works if read from a batch file, so something in the cmd line parser!
