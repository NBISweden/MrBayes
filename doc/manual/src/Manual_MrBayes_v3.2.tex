...
\ttt{primates.nex.run2.p}}. The topology and branch lengths are printed to a \ttt{.t} file; a Nexus
tree file that can be imported into programs like PAUP* \citep{swofford98}, Mesquite, and
FigTree~\footnote{In our case, \ttt{primates.nex.run1.t} and \ttt{primates.nex.run2.t}}. The root
of the \ttt{.p} and \ttt{.t} file names can be altered using the \ttt{Filename} setting.
...
using the \texttt{ratepr} parameter of the \ttt{prset} command. By default, \texttt{ratepr} is set
...
generations and summarize the obtained samples using \texttt{sump} (we also use \ttt{set
...
we use the \ttt{sumt} command to examine the calibrated tree:
...
the file with the ending \ttt{.con.tre} in a format that can be understood by the program FigTree.
